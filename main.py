...
# 确保 URL 包含协议头
if not COURSE_URL.startswith('http://') and not COURSE_URL.startswith('https://'):
    COURSE_URL = 'https://' + COURSE_URL
    print(f'自动添加协议头: {COURSE_URL}')

# 提取主页URL
try:
    from urllib.parse import urlparse
    parsed_url = urlparse(COURSE_URL)
    homePageURL = f'{parsed_url.scheme}://{parsed_url.netloc}/'
except Exception as e:
    print(f'URL解析失败: {e}')
    print('请确保course_url格式正确，例如: https://www.yuketang.cn/...')
    input('按回车键退出...')
    sys.exit(1)
